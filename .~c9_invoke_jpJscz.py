...
@app.route("/addTodo")
def addTodo():
    db.execute("INSERT INTO todos (user_id, todo, category) VALUES (:id, :todo, :category)", id=session["user_id"], todo=request)
    print
    # lists = [request.args.get("list1"),request.args.get("list2"),request.args.get("list3"),request.args.get("list4")]
    # db.execute("DELETE from todos WHERE user_id = :id", id = session["user_id"])
    # for list in lists:
    #     for index,todo in enumerate(list):
    #         db.execute("INSERT INTO todos (user_id, todo, category) VALUES (:id, :todo, :index)", id=session["user_id"], todo=list[index], index=index)

@app.route("/save")
def save():
    db.execute("INSERT INTO todos (user_id, todo, category) VALUES ('3', 'TEST SUCCESS', '1')")
